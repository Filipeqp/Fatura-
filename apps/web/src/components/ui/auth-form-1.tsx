...
interface AuthProps extends React.ComponentProps<"div"> {
  /** Qual tela exibir ao montar o componente. Útil para linkar direto em /login ou /registrar. */
  initialView?: "sign-in" | "sign-up";
}

function Auth({ className, initialView = "sign-in", ...props }: AuthProps) {
  const [state, setState] = React.useState<AuthState>({
    view: initialView === "sign-up" ? AuthView.SIGN_UP : AuthView.SIGN_IN,
  });
...
interface GoogleFlowState {
  status: "preparing" | "idle" | "loading" | "error";
  message?: string;
}

function AuthSocialButtons({ isLoading }: AuthSocialButtonsProps) {
  const tokenClientRef = React.useRef<{ requestAccessToken: () => void } | null>(null);
  const [google, setGoogle] = React.useState<GoogleFlowState>({ status: "preparing" });
  const { login } = useAuth();
  ...
  React.useEffect(() => {
    ...
    loadGoogleIdentityServices()
      .then((googleApi) => {
        if (cancelled) return;
        tokenClientRef.current = googleApi.accounts.oauth2.initTokenClient({
          client_id: import.meta.env.VITE_GOOGLE_CLIENT_ID,
          scope: "openid email profile",
          callback: async (response) => {
            if (response.error || !response.access_token) {
              setGoogle({ status: "error", message: "Login com Google cancelado ou negado" });
              return;
            }
            ...
            try {
              ...
              setGoogle({
                status: "error",
                message: err instanceof Error ? err.message : "Não foi possível entrar com o Google",
              });
            }
          },
        });
      ...
      .catch(() => {
        if (!cancelled) {
          setGoogle({ status: "error", message: "Não foi possível carregar o login do Google" });
        }
      });
  ...
  const busy = isLoading || google.status === "preparing" || google.status === "loading";
  ...
  return (
    <div data-slot="auth-social-buttons" className="w-full mt-6">
      ...
      {google.status === "error" && (
        <p className="mt-2 text-center text-xs text-destructive">{google.message}</p>
      )}
    </div>
  );
}
...
function AuthSeparator({ text = "Ou continue com" }: AuthSeparatorProps) {
  return (
...
function AuthSignIn({ onForgotPassword, onSignUp }: AuthSignInProps) {
  ...
  const onSubmit = async (data: SignInFormValues) => {
    ...
    try {
      ...
      setFormState((prev) => ({
        ...prev,
        error: err instanceof Error ? err.message : "Não foi possível entrar. Tente novamente.",
      }));
    } finally {
...
function AuthSignUp({ onSignIn }: AuthSignUpProps) {
  ...
  const onSubmit = async (data: SignUpFormValues) => {
    ...
    try {
      ...
      setFormState((prev) => ({
        ...prev,
        error: err instanceof Error ? err.message : "Não foi possível criar sua conta. Tente novamente.",
      }));
    } finally {
      setFormState((prev) => ({ ...prev, isLoading: false }));
    }
  ...
  return (
    ...
    >
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Comece a organizar suas faturas</p>
      </div>
