...
function AuthSignIn({ onForgotPassword, onSignUp }: AuthSignInProps) {
  ...
  const onSubmit = async (data: SignInFormValues) => {
    ...
    try {
      ...
      setFormState((prev) => ({ ...prev, error: "Não foi possível entrar. Tente novamente." }));
    } finally {
...
function AuthSignUp({ onSignIn }: AuthSignUpProps) {
  ...
  const onSubmit = async (data: SignUpFormValues) => {
    setFormState((prev) => ({ ...prev, isLoading: true, error: null }));
    try {
      // TODO: integrar com POST /auth/register
      await new Promise((resolve) => setTimeout(resolve, 1000));
      console.log("sign-up", data);
    } catch {
      setFormState((prev) => ({ ...prev, error: "Não foi possível criar sua conta. Tente novamente." }));
    } finally {
      setFormState((prev) => ({ ...prev, isLoading: false }));
    }
  };
...
function AuthForgotPassword({ onSignIn, onSuccess }: AuthForgotPasswordProps) {
  ...
  const onSubmit = async (data: ForgotPasswordFormValues) => {
    ...
    try {
      ...
      setFormState((prev) => ({ ...prev, error: "Ocorreu um erro inesperado" }));
    } finally {
      setFormState((prev) => ({ ...prev, isLoading: false }));
    }
  };
